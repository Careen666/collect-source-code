function [classifier, err_count, run_time, mistakes, mistakes_idx, SVs, TMs] = STSD_per(X, Y, NumFeature, id_list, C,lambda,algorithm)
...
%        w_new=tao_t*y_t*x_t(length(w_t)+1:end);
       w_new=y_t*x_t(length(w_t)+1:end);
       w_t=[w_t;w_new];
       w_t = w_t *min(1,1/(sqrt(lambda)*sum(abs(w_t))));
       w_t=truncate(w_t,NumFeature);
       flag=0;
