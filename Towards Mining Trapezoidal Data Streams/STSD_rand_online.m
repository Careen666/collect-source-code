function [w_t, err_count,time] = STSD_rand_online(NumFeature, C,lambda,algorithm, dimension,dataset,w_t,instance)
...
step=max(1,floor(1/length(dimen)*instance));
    ...
    if y_t*f_t<=1,
       ...
       v_idx=zeros(dimen_t,1);
       perm_t=randperm(dimen_t);
       c_t=perm_t(1:B);
       v_idx(c_t)=1;
       
       
       b=w_t(1:dimen_t,1).*v_idx;
       w_t=[b; w_t(dimen_t+1:end,1)];
